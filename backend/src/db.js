...
function seedInitialData() {
  ...
  // Kiểm tra nếu chưa có danh mục & menu
  ...
  if (catCount === 0) {
    ...
    // Category 1: Cà Phê Truyền Thống
    ...
    // Category 5: Bánh Ngọt & Tráng Miệng
  ...
  if (tableCount < 20) {
    ...
    // Xóa bàn cũ nếu chưa đủ 20 để seed lại đầy đủ 20 bàn Tầng 1 & Tầng 2
    if (tableCount > 0 && tableCount < 20) {
      db.prepare('DELETE FROM tables').run();
    }

    // Tầng 1: Bàn 01 -> Bàn 10
    ...
    // Sân Vườn & VIP
    insertTable.run('Bàn SV-1', 'Sân Vườn (Ngoài trời)', 4, 'EMPTY');
    insertTable.run('Bàn SV-2', 'Sân Vườn (Ngoài trời)', 4, 'EMPTY');
    insertTable.run('Bàn VIP-1', 'Khu VIP Hộp', 6, 'EMPTY');
    insertTable.run('Bàn VIP-2', 'Khu VIP Hộp', 10, 'EMPTY');
  }
}
